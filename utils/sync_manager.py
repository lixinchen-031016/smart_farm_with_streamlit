...
class DatabaseSyncManager:
    ...
    def sync_table_data(self, table_class, table_name, timestamp_column='timestamp'):
        ...
        try:
            ...
            log_operation(st.session_state.get('username', 'system'), "INFO",
                          "时间戳检查",
                          f"表 {table_name} - 云端最大时间戳: {cloud_max_time}, 本地最大时间戳: {local_max_time}")
...
def sync_databases_ui():
    ...
    cloud_db_port = st.number_input("云端数据库端口", min_value=1, max_value=65535,
                                    value=int(os.getenv('CLOUD_DATABASE_PORT', 3306)))
    ...
    local_db_port = st.number_input("本地数据库端口", min_value=1, max_value=65535,
                                    value=int(os.getenv('LOCAL_DATABASE_PORT', 3306)))
    ...
    if st.button("开始同步"):
        ...
        try:
            with st.spinner("正在进行数据同步..."):
                ...
                log_operation(st.session_state.get('username', 'system'), "INFO",
                              "数据库同步",
                              f"同步完成 - 云端→本地: {total_cloud_to_local}, 本地→云端: {total_local_to_cloud}")
