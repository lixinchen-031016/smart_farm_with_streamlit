    ...
    if len(numeric_columns) > 0:
        ...
        for col in numeric_columns:
            if col in desc_stats.columns:
                ...
                # 四分位数解释
                st.write(
                    f"  • 数据分布：25%的数据低于{q25_val:.2f}，50%的数据低于{q50_val:.2f}，75%的数据低于{q75_val:.2f}")
...
def explain_correlation_analysis(corr_matrix, data):
    ...
    if moderate_correlations:
        ...
        for col1, col2, corr in moderate_correlations:
            ...
            if 'temperature' in col1.lower() and 'humidity' in col2.lower():
                st.write(
                    f"  - 温度与湿度存在一定的{direction}关系，{'温度升高时湿度也倾向于上升' if corr > 0 else '温度升高时湿度倾向于下降'}")
            elif 'soil' in col1.lower() and 'humidity' in col2.lower():
    ...
    # 相关性分析总结
    total_pairs = len(very_strong_correlations) + len(strong_correlations) + len(moderate_correlations) + len(
        weak_correlations)
    st.markdown(f"\n**相关性分析总结**：")
    ...
    for col in numeric_columns:
        if 'temperature' in col.lower() or '温' in col:
            ...
            if moist_range[0] < 0 or moist_range[1] > 100:
                insights.append(
                    f"土壤湿度数据超出正常范围（{moist_range[0]:.1f}% ~ {moist_range[1]:.1f}%），请确认数据有效性")
