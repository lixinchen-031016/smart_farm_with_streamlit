...
def create_line_chart(data_list, title, y_title, threshold=None):
    ...
    if threshold:
        if isinstance(threshold, (list, tuple)):
            fig.add_hline(y=threshold[0], line_dash="dash", line_color="red",
                          annotation_text=f"最低阈值 {threshold[0]}")
            fig.add_hline(y=threshold[1], line_dash="dash", line_color="red",
                          annotation_text=f"最高阈值 {threshold[1]}")
        else:
...
def render_header(session):
    ...
    with st.container():
        ...
        st.markdown(
            f'<div class="{status_class}">📡 系统状态: <b>{status_text}</b> | ⏱️ 最后更新: <b>{last_update}</b> | 🌐 连接状态: <b>{connection_text}</b></div>',
