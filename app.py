...
def data_preview():
    ...
    if st.button("🔄 实时更新数据", help="点击获取最新传感器数据"):
        ...
        with col1:
            ...
            alert_card = '<div class="alert-card"></div>' if is_temp_alert else ''
            st.markdown(f'<div class="alert-container">{alert_card}', unsafe_allow_html=True)
            st.metric(label="🌡️ 空气温度", value=f"{air_temp_hum.temperature} °C",
                      delta="正常" if not is_temp_alert else "异常",
                      help="适宜范围：20°C - 30°C")
            st.markdown('</div>', unsafe_allow_html=True)
        with col2:
            is_humidity_alert = not (40 <= air_temp_hum.humidity <= 70)
            alert_card = '<div class="alert-card"></div>' if is_humidity_alert else ''
            st.markdown(f'<div class="alert-container">{alert_card}', unsafe_allow_html=True)
            st.metric(label="💧 空气湿度", value=f"{air_temp_hum.humidity} %",
                      delta="理想" if not is_humidity_alert else "注意",
                      help="适宜范围：40% - 70%")
            st.markdown('</div>', unsafe_allow_html=True)
        with col3:
            is_soil_moist_alert = not (30 <= soil_moist.value <= 60)
            alert_card = '<div class="alert-card"></div>' if is_soil_moist_alert else ''
            st.markdown(f'<div class="alert-container">{alert_card}', unsafe_allow_html=True)
            st.metric(label="🌱 土壤湿度", value=f"{soil_moist.value} %",
                      delta="适宜" if not is_soil_moist_alert else "需灌溉",
                      help="适宜范围：30% - 60%")
            st.markdown('</div>', unsafe_allow_html=True)
        with col4:
            is_nutri_alert = not (10 <= soil_nutri.value <= 20)
            alert_card = '<div class="alert-card"></div>' if is_nutri_alert else ''
            st.markdown(f'<div class="alert-container">{alert_card}', unsafe_allow_html=True)
            st.metric(label="🌱 土壤无机盐含量", value=f"{soil_nutri.value} ppm",
                      delta="正常" if not is_nutri_alert else "需施肥",
        ...
        with col5:
            ...
            alert_card = '<div class="alert-card"></div>' if is_light_alert else ''
            st.markdown(f'<div class="alert-container">{alert_card}', unsafe_allow_html=True)
            st.metric(label="☀️ 光照强度", value=f"{light_intens.value} lux",
                      delta="充足" if not is_light_alert else "不足",
